...
def click(objct):  # Обработчик нажатия на блок (квадрат)
    ...
    if not win:
        if latest == 'x':
            ...
            if player1.go(board, SqrtList.index(objct)):
                ...
                latest = 'x'
        if board.end_of_game(latest) == 1:
            if latest == 'x':
                InfoLabel.config(text='Выиграл Игрок 1!')
                ErrorLabel.config(text='Мои поздравления!')
                win = True
            else:
                InfoLabel.config(text='Выиграл Игрок 2!')
                ErrorLabel.config(text='Мои поздравления!')
        elif board.end_of_game(latest) == 2:
            InfoLabel.config(text='Ничья!')
            ErrorLabel.config(text='Эх, придётся переигрывать')
            win = True
...
def sqrt_change_color(list, c, color):
    for obj in list:
...
root.title('Крестики-нолики v. 2.1 (BETA)')
root.resizable(False, False)

TitleLabel = Label(text='Это игра Крестики-нолики v. 2.1\n Первый игрок - крестик, второй - нолик',
                   font='Consolas, 14')
TitleLabel.pack()
...
polefontm.add_command(label='Зелёный', command=lambda c='green': sqrt_change_color(SqrtList, canv, 'green'))
